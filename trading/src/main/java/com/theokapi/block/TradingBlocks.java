...
public class TradingBlocks {
    private static Block register(String name, Function<BlockBehaviour.Properties, Block> blockFunction, BlockBehaviour.Properties blockProperties) {
        ResourceKey<Block> blockResourceKey =
                ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath(Trading.MOD_ID, name));

        Block block = blockFunction.apply(blockProperties.setId(blockResourceKey));

        ResourceKey<Item> itemResourceKey =
                ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(Trading.MOD_ID, name));

        BlockItem blockItem =
                new BlockItem(block, new Item.Properties().setId(itemResourceKey).useBlockDescriptionPrefix());
    ...
    public static final Block TRADING_TABLE = register(
            "trading_table",
            TradingTableBlock::new,
